...
( function ( $ ) {
	$( document ).ready( function () {
		...
		if ( isMobile == null && container.length ){
			...
			while( baseNode.css && (baseNode.css('background-color') == 'rgba(0, 0, 0, 0)'||baseNode.css('background-color') == 'transparent')) {
		...
		function getpos( element ) {return element.offset().top-$(window).scrollTop();}

		function gethead() {return contents.find( "[id='"+window.location.hash.substr(1)+"']:first" );}

		function AdjustHeader( e ){
			/* h2 */
			var clopos = fade, opacity;
			...
			container.children().eq(0).css({"opacity":opacity, display:opacity>0?'':'none'});
			...
			if ( lower ){
				...
				container.children().eq(1).css({"opacity":opacity, display:opacity>0?'':'none'});
			}
			if ( e == null ){
				var base = gethead();
